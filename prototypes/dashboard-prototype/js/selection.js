/** Copied and modified from https://github.com/Simonwep/selection */
...
function Selection(options = {}) {
  const that = {
    ...
    _init() {
      that._selectionAreaContainer = selectAll(
    ...
    _onSingleTap(evt) {
      ...
      if (evt.shiftKey && stored.length) {
        ...
        const [preceding, following] =
          middle.compareDocumentPosition(target) & 4
            ? [target, middle]
            : [middle, target];
    ...
    _delayedTapMove(evt) {
      ...
      if (
        abs(x + y - (that._areaX1 + that._areaY1)) >=
        that.options.startThreshold
      ) {
    ...
    _onTapMove(evt) {
      ...
      if (that._scrollAvailable && (ss.y !== null || ss.x !== null)) {
        ...
        requestAnimationFrame(function scroll() {
          ...
          if (ss.x !== null) {
            scon.scrollLeft += Math.ceil(
              ss.x / that.options.scrollSpeedDivider
            );
      ...
      if (x < brect.left) {
        ...
        ss.x =
          scrollWidth - scrollLeft - clientWidth
            ? Math.abs(brect.left + brect.width - x)
            : null;
      ...
      if (y < brect.top) {
        ...
        ss.y =
          scrollHeight - scrollTop - clientHeight
            ? Math.abs(brect.top + brect.height - y)
            : null;
    ...
    _updatedTouchingElements() {
      ...
      // Filter event
      const filterEvent =
        options.selectionFilter && options.selectionFilter.bind(that);
      ...
      for (
        let i = 0, n = _selectables.length, node;
        (node = _selectables[i]), i < n;
        i++
      ) {
      ...
      for (
        let i = 0, n = _touchedElements.length, el;
        (el = _touchedElements[i]), i < n;
        i++
      ) {
    ...
    keepSelection() {
      for (
        let i = 0, n = that._touchedElements.length, el;
        (el = that._touchedElements[i]), i < n;
        i++
      ) {
    ...
    select(query) {
      ...
      const elements = selectAll(query).filter(
        el => !_touchedElements.includes(el) && !_selectedStore.includes(el)
      );
